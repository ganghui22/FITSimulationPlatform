...
def mark_location():
	import  numpy as np
	location_list={'elevator_1':(14.30,-51.42),'elevator_2':(10.10,-3.37),'文栋':(81.05,13.98),'meeting':(74.90,10.18),'刘老师':(69.55,-69.32)}
	src = cv2.imread('./PathPlanningAstar/fit4_5Dealing_draw.png')
	for key,value in location_list.items():
		text=key
		cv2.putText(src, text, world_to_pixel(world_points=(value[0], value[1]-5),image_size=(2309,2034)), cv2.FONT_HERSHEY_COMPLEX, 1.0, (0,0,255), 2)
...
class Graph():
	def __init__(self):
		self.person=['港晖','晨峻','伟华','刘老师','袁老师','刘毅','姚峰','侯煊','小飞','郝伟','海洋','春秋','靖宇','兴航','文栋','兰军','李老师','馨竹']
		self.location = ['Room510', 'Room511', 'Room512', 'Room513', 'Room514', 'Room515', 'Room516']
		self.other_location=['1号会议室','2号会议室','茶水间','1001教室','1002教室','1003教室','讨论区','休息室']
		self.outdoor=['超市','电影院','上海','公园','机场','食堂']
		self.pos_1={
		'港晖': [-0.06590949, -0.98459114], 'Room516': [-0.11786665, -0.85143201], '晨峻':
			...
			[-0.81032829, 0.01610876], '2号会议室': [0.84768565, -0.59677], '茶水间':
			[-0.64787042, 0.60134683], '1001教室': [0.89010407, -0.27253654], '1002教室':
			[-0.92707715, -0.29802952], '1003教室': [0.80476599, 0.56320228], '讨论区':
			[1., 0.02354849], '休息室': [-0.74027552, -0.46807695],'其他':[0.074027552, -0.46807695]}
		self.man_anotate={'Room511': [-0.58564247, 0.25416477], 'Room512': [-0.38256115, -0.57270125],'Room513': [0.33214105, -0.87734133],'Room515': [0.57428342, -0.38469979],'Room514': [0.22919887, 0.65937288],'Room510': [-0.5283078, 0.56331821],'Room516': [0.43335628, 0.92137264]}
	def draw(self,graph_rel):
		...
		for personName,all_rel in self.graph_rel.items():
			if all_rel['rel_now']!=None:
				...
				self.G.add_edge(all_rel['rel_base'][0],all_rel['rel_base'][1],edge_color=all_rel['rel_base'][2]*1000)
		...
		nx.draw_networkx(self.G, with_labels=False, width=2,pos=self.pos,node_color=self.node_colors,edge_color=self.edge_colors,edge_cmap=plt.cm.Blues)  # add colors
...
class update():
	def __init__(self):
		...
		self.time_propity={'1号会议室':30,'2号会议室':30,'休息室':50,'其他':30,'1001教室':30,'1002教室':30}
		self.messege=None
		self.waiting_update=[]
		with open('Graph/Graph.json', 'r') as load_f:
		...
		while 1:
			...
			for k ,info in self.need_update.items():
				...
				if time_err<self.total_time_o:
					tmp_possibillity=cal_time(self.a_time, self.total_time_o, self.u, self.sigma,time_err)*self.sigma
					self.graph_rel[info[0]]['rel_now'][2]=tmp_possibillity#更新可能性
					# print(stamptotime(self.now_time),self.graph_rel[info[0]]['rel_now'])
				#小于阈值的时候，相当于不再存在
				if time_err>self.total_time_o or (time_err>100 and self.graph_rel[info[0]]['rel_now'][2]<0.07):#这里加上time——err的
